...
- Having a Babel compiler by downloading it from unpkg.com and having it JUST WORK within the browser is... magic. ✨
- Interpolation in JSX requires `{}` only, unlike the interpolation for JS which requires `${}` surrounded by backticks.
- If you declare a constant that's assigned to a string value, there's nice syntatic sugar to create an object literal with that constant as the key and its string as its value:

```js
// Consider:
    const className = 'container'
    const children = 'Hello World'
    const elementProps = { children, className }

// Or you could do this:
const elementProps = {
  children: 'container',
  className: 'Hello World', /* trailing comma optional ;) */
}
```
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=03%3A%20Using%20JSX&em=francisfuzz%40github.com15894826%2Bfrancisfuzz%40users.noreply.github.com).
