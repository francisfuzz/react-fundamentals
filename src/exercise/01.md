...
- This is my second time going through the exercise.
- It's for an HTML element, it's possible to set its `id` in object accessor fashion (`someElement.id = 'unique'`) or by invoking the setter (`someElement.setAttribute('id', 'unique')`). They both accomplish the same thing, but a big part of me prefers invoking the setter because of how descriptive it is.
- The extra credit taught me that it's possible to build out an application entirely in JavaScript so long as the `<body>` already exists.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=01%3A%20Basic%20JavaScript-rendered%20Hello%20World&em=francisfuzz%40github.com15894826%2Bfrancisfuzz%40users.noreply.github.com).
