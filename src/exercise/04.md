...
- Destructuring is the coolest. In a function that accepts an object as its only argument, you can use destructuring to "pull" keys from said object in the body of the function:

```jsx
function message ({children}) {
  // The actual return value is something that can only be done in JSX given the `<div>` present.
  return <div className="message">{children}</div>
}
```

- I love that JavaScript treats functions as a first-class object. We can pass a function to the first element of `React.createElement`, be it one that's already defined, or an anonymous one, and the React will take what's specified in the props and pass it into said function:

```jsx
// Assume that the `message` passed here is the earlier function in the notes
const element = (
  <div className="container">
    {React.createElement(message, {children: 'Hello World'})}{' '}{React.createElement(message, {children: 'Goodbye World'})}
  </div>
)
```
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=04%3A%20Creating%20custom%20components&em=francisfuzz%40github.com15894826%2Bfrancisfuzz%40users.noreply.github.com).
